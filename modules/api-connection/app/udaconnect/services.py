...
class ConnectionService:
    ...
    def find_contacts(person_id: int, start_date: datetime, end_date: datetime, meters=5
        ...
        """
        # Find all locations of a person
        locations: List = db.session.query(Location).filter(Location.person_id == person_id)\
        .filter(Location.creation_time < end_date)\
        .filter(Location.creation_time >= start_date).all()

        # Cache all users in memory for quick lookup
        person_map: Dict[str, Person] = {person.id: person for person in person_stub.GetPersons(person_pb2.Request())}
